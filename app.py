...
st.caption("Persistencia real en Supabase (PostgreSQL). Guardá con el botón 💾 Guardar (upsert).")
...
with st.expander("Abrir editor", expanded=True):
    ...
    if c3.button("💾 Guardar (upsert)"):
        # upsert de TODO lo que está en la grilla
        main.upsert_tasks(st.session_state["df_full"])
        st.success("Cambios guardados en Supabase.")
        # refrescar para traer IDs autogenerados y normalizar
        st.session_state["df_full"] = main.fetch_tasks()
...
st.caption("Conectado a Supabase usando st-supabase-connection.")
